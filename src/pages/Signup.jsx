  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://tmbill-backend.onrender.com/api/v1/sign-up",
        data
      );
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          {["username", "email", "password"].map((field) => (
            <div className="relative" key={field}>
              ...
              <label
                htmlFor={field}
                className="absolute left-3 top-4 text-gray-400 text-sm transition-all peer-placeholder-shown:top-3.5 peer-placeholder-shown:text-base peer-placeholder-shown:text-gray-500 peer-focus:top-2.5 peer-focus:text-sm peer-focus:text-blue-400"
              >
                {data[field].length
                  ? null
          ...
          <button
            type="submit"
            disabled={loading}
            className={`w-full bg-blue-600 hover:bg-blue-500 cursor-pointer text-white font-semibold py-3 rounded-lg transition-all ${
